...
export default class ColorPicker extends Component {
  ...
  constructor() {
    ...
    this.drawSCanvas = this.drawSCanvas.bind(this);
    this.hsvToRgb = this.hsvToRgb.bind(this);
    this.redrawCanvases = this.redrawCanvases.bind(this);
  }
  ...
  componentDidMount() {
    let hvc = this.base.children[0].children[0];
    let hvctx = hvc.getContext('2d');
    let sc = this.base.children[0].children[1];
    hvctx.fillStyle = this.props.baseColor.slice(0);
    let hvcol = this.rgbToHsv(this.hexToRgb(hvctx.fillStyle));
    let calcWidth = Math.floor(this.base.offsetWidth * .25);
    let calcHeight = this.base.children[0].children[3].offsetHeight;
    hvc.height = calcHeight;
    hvc.width = calcWidth;
    sc.height = calcHeight;
    this.base.children[0].children[2].height = calcHeight;
    this.setState({
      ...
      canvasHeight: this.base.children[0].children[3].offsetHeight,
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.baseColor !== this.state.baseColor) {
      this.setState({baseColor: nextProps.baseColor.slice(0)});
    }
  }

  componentWillUpdate(nextProps,nextState){
    if (this.state.baseColor !== nextProps.baseColor) {
      const color = nextProps.baseColor.slice(0);
  ...
  redrawCanvases() {
    this.drawHVCanvas(this.base.children[0].children[0]);
    this.drawSCanvas(this.base.children[0].children[1]);
    this.drawPCanvas(this.base.children[0].children[2]);
  }
  ...
  hueValLocationChange(evt) {
    ...
    if (evt.target !== this.base.children[0].children[0]) return;
  ...
  satLocationChange(evt) {
    if (evt.srcEvent) evt = evt.srcEvent;
    if (evt.target !== this.base.children[0].children[1]) return;
  ...
  resetColor(e) {
    ...
    let hvctx = hvc.getContext('2d');
    let sc = this.base.children[0].children[1];
    hvctx.fillStyle = this.state.baseColor;
    let hvcol = this.rgbToHsv(this.hexToRgb(hvctx.fillStyle));
    this.setState({
      baseColor: hvctx.fillStyle,
      newColor: hvctx.fillStyle,
  ...
  setFeature(e) {
    e.preventDefault();
    const color = this.state.newColor.slice(0);
    this.setState({baseColor:color});
    this.props.onColorUpdate(this.props.index,color);
  }
  ...
  render(props, state) {
    ...
    return (
      <div class={'pure-control-group'}>
        <div
          id={'horizontal-layout-container'} style={{margin:'1em'}}
        >
          {hueValCanvas}
          {satCanvas}
          {previewCanvas}
          <div style={{display: 'inline-block'}}>
            <input
              id={'color-input'}
              type={'text'}
              onInput={(e)=>this.onManualColor(e)}
              value={newColor}
              style={{width: '8rem'}}
            />
            <button
            ...
            <button
              ...
              onClick={(e)=>this.resetColor(e)}
            >
              Reset
            </button>
            <button
              class={'color-select-ctrl'}
              onClick={(e)=>this.removeFeature(e)}
            >
              Remove
            </button>
          </div>
